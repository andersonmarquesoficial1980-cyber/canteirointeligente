...
import { ArrowLeft, Plus, Building2, Users, ChevronRight, CheckCircle, AlertTriangle, Clock, X, Settings } from "lucide-react";
...
export default function SSTObrasIntegracao() {
  ...
  async function salvarObra() {
    if (!formObra.nome_obra) return;
    setSalvandoObra(true);
    const { data: { user } } = await supabase.auth.getUser();
    await supabase.from("sst_obras_integracao").insert({
  ...
  async function salvarFuncionario() {
    ...
    // Calcular data_vencimento automaticamente
    ...
    await supabase.from("sst_funcionarios_integracao").insert({
      company_id: COMPANY_ID,
  ...
  async function salvarEdicaoFunc() {
    ...
    await supabase.from("sst_funcionarios_integracao").update({
  ...
  function toggleDoc(doc: string, lista: string[], setLista: (v: string[]) => void) {
    setLista(lista.includes(doc) ? lista.filter(d => d !== doc) : [...lista, doc]);
  }
  ...
  return (
    <div className="min-h-screen bg-background">
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", gap: 12, marginTop: 5, flexWrap: "wrap" }}>
                          {fi.data_integracao && (
                            <span style={{ fontSize: 11, color: "#64748b" }}>
                              Integrado: {fmtDate(fi.data_integracao)}
                            </span>
                          )}
                          {fi.data_vencimento && (
      ...
      {/* Modal: Nova Obra */}
      <Dialog open={modalObra} onOpenChange={setModalObra}>
        <DialogContent className="max-w-sm mx-4 rounded-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <Button onClick={salvarObra} disabled={salvandoObra || !formObra.nome_obra}
              ...
              {salvandoObra ? "Salvando..." : "Criar Obra"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modalFuncionario} onOpenChange={setModalFuncionario}>
        <DialogContent className="max-w-sm mx-4 rounded-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
              ...
              <Select value={funcSelecionadoId} onValueChange={setFuncSelecionadoId}>
                ...
                <SelectContent>
                  {funcionariosDisponiveis.map(f => (
                    <SelectItem key={f.id} value={f.id}>{f.name} — {f.matricula}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <Button onClick={salvarFuncionario} disabled={salvandoFunc || !funcSelecionadoId}
              className="w-full h-11 rounded-xl font-display font-bold">
              {salvandoFunc ? "Salvando..." : "Adicionar Funcionário"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
