...
begin
  ...
  loop
    ...
    -- Fallback: nome com token abreviado (ex.: "P" no cadastro x "PEREIRA" no PDF)
    if v_employee_id is null then
      with src as (
        select array_remove(regexp_split_to_array(regexp_replace(lower(v_nome), '[^a-z0-9 ]', '', 'g'), '\\s+'), '') as t
      )
      select e.id
        into v_employee_id
      from public.employees e
      cross join src
      cross join lateral (
        select array_remove(regexp_split_to_array(regexp_replace(lower(coalesce(e.name, '')), '[^a-z0-9 ]', '', 'g'), '\\s+'), '') as te
      ) norm
      where e.company_id = v_job.company_id
