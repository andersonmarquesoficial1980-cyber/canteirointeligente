-- RPC segura para edição em lote de equipamentos no Dashboard de Frotas
-- Campos permitidos: status, setor, local_atual, valor_mensal
-- Regras: somente usuários com permissão de admin_section.maquinas (manage/edit)
--         ou all/manage, respeitando user_admin_panel_access.

DROP FUNCTION IF EXISTS public.update_equipamentos_dashboard_lote(uuid[], text, text, text, text, numeric);
...
BEGIN
  ...
  -- superadmin/admin do perfil: acesso liberado
...
GRANT EXECUTE ON FUNCTION public.update_equipamentos_dashboard_lote(uuid[], text, text, text, text, numeric) TO authenticated;
