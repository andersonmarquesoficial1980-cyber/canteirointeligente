-- PREVIEW (read-only) status sync seguro planilha -> workflux
...
select u.target_status, e.status as status_atual, count(*) as qtd
from upd u join employees e on e.id=u.employee_id::uuid
group by u.target_status,e.status
order by qtd desc;
...
select e.id,e.name,e.matricula,e.cpf,e.status as status_atual,u.target_status
from upd u join employees e on e.id=u.employee_id::uuid
order by e.name
limit 200;
